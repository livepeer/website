...
**Related documents:**
- [`pipeline-sdk.md`](./pipeline-sdk.md) — the developer SDK providers use to push pipelines onto the network. Studio auto-generates catalog entries and playgrounds from the SDK's schema.
- [`pipeline-sdk-architecture.md`](./pipeline-sdk-architecture.md) — ADR for the SDK and its BYOC contract alignment.
- [`runtime.md`](./runtime.md) — orchestrator-side container isolation, GPU tiers, and worker filtering. Relevant when Studio routes requests to the network.
...
### Page 3: Network Stats (`/studio/stats`) — NO ACCOUNT REQUIRED

Public page that gives developers confidence the network is real, active, and capable. Think Chutes' Statistics + Nodes pages, stripped of anything governance-related (staking, rounds, delegation — that's on the Explorer).

**Design principle:** Simple numbers developers immediately understand. No protocol jargon. Every metric answers one of three questions: "Is this network alive?", "Can it handle my workload?", or "What does it cost?"

**Sub-navigation (left sidebar or tabs):**

```
Stats
├── Overview       ← Is this network alive?
├── GPUs           ← Can it handle my workload?
└── Pipelines      ← What's available and what does it cost?
```

#### Stats > Overview

The "social proof" page. Developers land here to decide if the network is worth evaluating.
...
│                                                                   │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐           │
│  │ Total    │ │ Active   │ │ Success  │ │ Orchest- │           │
│  │ Requests │ │ GPUs     │ │ Rate     │ │ rators   │           │
│  │ 12.4M    │ │ 312      │ │ 99.2%    │ │ 83       │           │
│  │ ▲ 18%    │ │ ▲ 8%     │ │ ▲ 0.3%   │ │ ▲ 5      │           │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘           │
│                                                                   │
│  TOTAL REQUESTS                                                   │
│  12.4M  Total Requests                                           │
│  Total inference requests across all APIs on the network.        │
│  Time Period: [Last 3 Months ▾]                                  │
│  ┌───────────────────────────────────────────────────────┐       │
│  │  (stacked bar chart over time, colored by API)        │       │
│  └───────────────────────────────────────────────────────┘       │
│                                                                   │
│  Top APIs (Last 3 Months)                                        │
│  ● SDXL Turbo  ● FLUX.1 schnell  ● Stable Video Diffusion ...   │
│                                                                   │
│  ACTIVE USERS                                                     │
│  3.2K  Total Users                                               │
│  Developers using the network via Studio API keys.               │
│  Time Period: [Last 3 Months ▾]                                  │
│  ┌───────────────────────────────────────────────────────┐       │
...
**Content:**
- KPI cards: Total Requests (30D, with delta), Active GPUs, Success Rate, Orchestrators Online
- Requests over time: Stacked bar chart colored by top APIs (like Chutes' Total Tokens chart). Time period selector (7D / 30D / 3M).
- Top APIs legend: Colored dots + names matching the chart (like Chutes' Top Models)
- Active users: Area chart showing developer adoption growth

**Data source (later):** NAAP `kpi` resolver for request counts/success rate, `pipelines` resolver for per-API breakdown, `orchestrators` resolver for orchestrator count.

#### Stats > GPUs

The "capacity" page. Developers need to know what hardware backs the network and whether it can scale.

```
┌─────────────────────────────────────────────────────────────────┐
│  GPU Capacity                                         1.18K     │
│                                                     ▲ 5.8%     │
│  [Total Nodes ●] [Nodes by GPU Type ○]                          │
│  ┌───────────────────────────────────────────────────────┐      │
│  │  (area chart — GPU node growth over 90 days)          │      │
│  └───────────────────────────────────────────────────────┘      │
│                                                                   │
│  GPU Distribution                                                │
│  ┌───────────────────────────────────────────────────────┐      │
│  │  (bubble chart — circle size = count, grouped by GPU) │      │
│  └───────────────────────────────────────────────────────┘      │
│                                                                   │
│  GPU Inventory                                                   │
│  │ GPU Name         │ Count │ Memory    │ TFLOPS │ Max Power │   │
│  │ NVIDIA H100 80GB │  724  │ 141 GB    │ 67     │ 700W      │   │
│  │ NVIDIA RTX A6000 │  103  │ 48 GB     │ 38.7   │ 300W      │   │
│  │ NVIDIA A100 80GB │   80  │ 80 GB     │ 19.5   │ 400W      │   │
│  │ ...              │       │           │        │           │   │
│  │ Total            │ 1,069 │           │        │           │   │
└─────────────────────────────────────────────────────────────────┘
```
...
**Content:**
- GPU node growth chart: Area chart showing total GPU count over 90 days, with toggle to break down by GPU type (like Chutes' Node Growth Analytics)
- GPU distribution visual: Bubble/circle chart showing relative GPU counts (like Chutes' GPU Node Counts)
- GPU inventory table: Sortable table with GPU Name, Count, Memory, Bandwidth, Performance (TFLOPS), Max Power (like Chutes' GPU Node Table)

**Data source (later):** NAAP `gpuCapacity` resolver for GPU counts, `net-capacity` resolver for detailed breakdown.

#### Stats > Pipelines

The "what's available" page. Developers check which AI capabilities are on the network and what they cost.

```
┌─────────────────────────────────────────────────────────────────┐
│  Pipelines                                                       │
│                                                                   │
│  │ Pipeline           │ Warm │ Price        │ Output/$1 │ Cap. │ │
│  │ text-to-image      │  47  │ $0.005/req   │ 200 imgs  │  72  │ │
│  │ image-to-image     │  38  │ $0.008/req   │ 125 imgs  │  64  │ │
│  │ image-to-video     │  29  │ $0.05/sec    │ 20 secs   │  45  │ │
│  │ live-video-to-video│  32  │ $0.006/min   │ 166 mins  │  52  │ │
│  │ text-to-speech     │  18  │ $0.002/req   │ 500 reqs  │  30  │ │
│  │ audio-to-text      │  22  │ $0.003/min   │ 333 mins  │  35  │ │
│  │ llm                │  15  │ $0.01/M tok  │ 100M tok  │  24  │ │
│  │ segment-anything   │  12  │ $0.004/req   │ 250 reqs  │  20  │ │
│                                                                   │
│  Pipeline Usage (30D)                                            │
│  ┌───────────────────────────────────────────────────────┐      │
│  │  (stacked area chart — usage minutes by pipeline)     │      │
│  └───────────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────┘
```

**Content:**
- Pricing table: Pipeline name, Warm orchestrator count, Price per unit, Output per dollar (value prop), Total capacity. Sortable columns.
- Usage chart: Stacked area chart showing usage minutes per pipeline over 30 days.
- Each pipeline row links to the Explore page filtered by that capability type.

**Data source (later):** NAAP `pricing` resolver for per-pipeline pricing, `pipelines` resolver for usage breakdown, `gpuCapacity.pipelineGPUs` for capacity.

**What's deliberately excluded from Stats:**
- Governance: rounds, block progress, inflation, participation rate (→ Explorer)
- Staking: total bonded LPT, delegator counts, reward cuts, fee shares (→ Explorer)
- Individual orchestrator details: specific addresses, staking positions (→ Explorer)
- Payment transactions: provider earnings, individual payments (→ Provider Dashboard)
- Miner/provider tools: My Miner, leaderboard rankings (→ future Provider section)

---

### Page 4: Dashboard (`/studio/dashboard`) — REQUIRES ACCOUNT
...
> The `runner/` submodule of this SDK is documented in [`pipeline-sdk.md`](./pipeline-sdk.md). The architectural rationale is in [`pipeline-sdk-architecture.md`](./pipeline-sdk-architecture.md). This appendix discusses the SDK at a higher level as one of Studio's possible network-facing backends.
...
### Network-API onboarding (via the Pipeline SDK)
For providers deploying directly onto the Livepeer network (not third-party managed services), the onboarding is much shorter: they author a Pipeline class, run `livepeer push`, and Studio auto-discovers the capability — no registration wizard needed. See [`pipeline-sdk.md`](./pipeline-sdk.md) for the full developer flow, and Appendix 13 below for how Studio renders the resulting catalog entry and playground.
...
## Appendix 13: Pipeline SDK Integration & Live Playground

This appendix describes how Studio integrates with the Livepeer Pipeline SDK and how it renders interactive playgrounds — both the auto-generated request-response form and the WebRTC live playground for real-time streaming APIs. For the full SDK spec (base classes, `Input()` descriptors, CLI), see [`pipeline-sdk.md`](./pipeline-sdk.md). For the ADR and BYOC contract alignment, see [`pipeline-sdk-architecture.md`](./pipeline-sdk-architecture.md).

### What Studio consumes from the SDK

When a provider runs `livepeer push`, the SDK generates a JSON Schema from the Pipeline class's `predict()` / `on_frame()` signature and registers the capability on the network. Studio consumes that schema to populate its catalog and playground — the provider writes zero frontend code.

**Pipeline → Studio mapping:**
...
```json
// Studio receives (via the /schema endpoint):
{
  "params": [
...
**Studio auto-generates from this schema:**
- **Catalog card** with name, description, pricing, latency (see Page 1 wireframe)
- **Playground form** — type-driven controls (string → text input, enum → dropdown, number → slider, file → upload, boolean → toggle)
...
### What happens on `livepeer push`

From Studio's point of view, the deployment flow is:

1. Provider runs `livepeer push my_pipeline.py`
2. SDK builds container, pushes to registry, registers capability on the network with the schema
3. Orchestrators opt in, pull the container, advertise the capability
4. Studio's `/api/v1/apis` endpoint detects the new network capability
5. Auto-creates a catalog entry with `provider_type = 'network'`
6. Playground auto-generated from the schema
7. Badge: `LIVEPEER · by ProviderName`

The provider goes from "I have a model" to "it's live on the network with a playground" in one push.

### Two playground types (driven by `interaction_type`)

Studio renders different playground components based on the `interaction_type` field in the schema:

**Type 1: Request-Response (`interaction_type: "request_response"`)**
...
**Type 2: Live / WebRTC (`interaction_type: "webrtc_stream"`)**
```
┌─────────────────────────────────────────────────────────────┐
│  ┌──────────────┐    ┌──────────────┐                       │
│  │  Input       │    │  Output      │                       │
...
For `StreamPipeline`, parameter fields become real-time sliders updated over the WebRTC data channel. Users see the effect of each change instantly.

### WebRTC playground implementation (~200 lines, one reusable component)
...
This is a single reusable React component. Every streaming API uses the same pattern — the only difference is the `live_params` schema that determines which sliders/inputs appear.
...
- Timer shown in the playground UI
- After 3 minutes: "Sign up to continue"

### Provider observability in Studio

There is no direct channel between provider and orchestrator — the registry is the intermediary, and Studio is the provider's operational surface:
...
### Timeline for Studio playground work

```
Week 1-2:   Request-response playground (auto-generated forms)
Week 3-4:   Live WebRTC playground component — works for Scope/Daydream and any future streaming API
```

### Why this matters for Studio

Every pipeline pushed via the SDK becomes a new API in Studio's catalog, with a working playground and code snippets, automatically. More pipelines → more developers using Studio → more compute demand → more network revenue. The SDK is Studio's supply-side growth engine — it turns Studio from "a portal with a curated catalog" into "a portal that grows whenever anyone ships a pipeline."
