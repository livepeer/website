    ...
    <div className="relative h-full overflow-hidden rounded-lg bg-[#070b0a]">
      ...
      >
        <g stroke="rgba(255,255,255,0.15)" strokeWidth="0.5">
          ...
          {/* Flower petals */}
          <ellipse cx="100" cy="38" rx="6" ry="12" transform="rotate(0 100 38)" />
          <ellipse cx="100" cy="38" rx="6" ry="12" transform="rotate(60 100 38)" />
          <ellipse cx="100" cy="38" rx="6" ry="12" transform="rotate(120 100 38)" />
          {/* Flower center */}
      ...
      >
        ...
        >
          ...
          <g stroke="rgba(64,191,134,0.7)" strokeWidth="1">
            ...
            <path d="M100,70 C110,62 118,64 120,60" fill="rgba(64,191,134,0.06)" />
            {/* Leaf veins */}
            <line x1="100" y1="80" x2="88" y2="73" />
            <line x1="100" y1="70" x2="112" y2="63" />
            {/* Flower petals */}
            <ellipse cx="100" cy="38" rx="6" ry="12" transform="rotate(0 100 38)" fill="rgba(64,191,134,0.08)" />
            <ellipse cx="100" cy="38" rx="6" ry="12" transform="rotate(60 100 38)" fill="rgba(64,191,134,0.08)" />
            <ellipse cx="100" cy="38" rx="6" ry="12" transform="rotate(120 100 38)" fill="rgba(64,191,134,0.08)" />
            {/* Flower center */}
            <circle cx="100" cy="38" r="5" fill="rgba(64,191,134,0.12)" />
          </g>
          {/* Glow particles — pollen/sparkles */}
          {[
            [92, 28], [108, 30], [88, 45], [112, 42], [95, 50],
            [105, 48], [85, 72], [115, 62], [90, 85], [110, 78],
          ].map(([x, y], i) => (
            <circle
              key={i}
              cx={x}
      ...
      <div
        ...
        style={{
          left: 0,
          background: "linear-gradient(to bottom, transparent, rgba(64,191,134,0.6) 30%, rgba(64,191,134,0.9) 50%, rgba(64,191,134,0.6) 70%, transparent)",
          boxShadow: "0 0 12px rgba(64,191,134,0.4), 0 0 24px rgba(64,191,134,0.2)",
  ...
  return (
    <div className="relative h-full overflow-hidden rounded-lg bg-[#0a0a0a]">
      ...
      <div className="absolute inset-3 flex items-center justify-center">
        <svg viewBox="0 0 280 120" className="w-full max-w-[300px] h-auto" fill="none">
          {/* Source node */}
          <rect x="10" y="45" width="50" height="26" rx="5"
            fill="rgba(24,121,78,0.25)" stroke="rgba(24,121,78,0.7)" strokeWidth="1"
            style={{ animation: "breathe 4s ease-in-out infinite" }}
          />
          <text x="35" y="61" textAnchor="middle" dominantBaseline="middle"
            className="font-mono" fontSize="8" fill="rgba(64,191,134,0.9)">
            Source
          </text>

          {/* Fork node */}
          <rect x="110" y="45" width="40" height="26" rx="5"
            fill="rgba(255,255,255,0.05)" stroke="rgba(255,255,255,0.12)" strokeWidth="1"
          />
          <text x="130" y="61" textAnchor="middle" dominantBaseline="middle"
            className="font-mono" fontSize="7" fill="rgba(255,255,255,0.5)">
            ABR
          </text>

          {/* Source → Fork connection */}
          <line x1="60" y1="58" x2="110" y2="58"
            stroke="rgba(255,255,255,0.15)" strokeWidth="1" strokeDasharray="4 4"
            style={{ animation: "dashFlow 1.5s linear infinite" }}
          />
          <circle r="2.5" cy="58" fill="#40bf86">
            <animate attributeName="cx" from="60" to="110" dur="1.2s" repeatCount="indefinite" />
            <animate attributeName="opacity" values="0;0.9;0.9;0" keyTimes="0;0.1;0.9;1" dur="1.2s" repeatCount="indefinite" />
          </circle>

          {/* Fork → Endpoints */}
          {endpoints.map((ep, i) => (
            <g key={i}>
              <line x1="150" y1="58" x2="230" y2={ep.y + 8}
                stroke="rgba(255,255,255,0.08)" strokeWidth="0.5"
              />
              {/* Endpoint node */}
              <rect x={ep.x - 10} y={ep.y} width="40" height="16" rx="3"
                fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.1)" strokeWidth="0.5"
              />
              <text x={ep.x + 10} y={ep.y + 10} textAnchor="middle" dominantBaseline="middle"
                className="font-mono" fontSize="6" fill="rgba(255,255,255,0.4)">
                {ep.label}
              </text>
              {/* Flowing packet */}
              <circle r="1.5" fill="#40bf86">
                <animate attributeName="cx" from="150" to="230" dur="2s" begin={`${i * 0.35}s`} repeatCount="indefinite" />
                <animate attributeName="cy" from="58" to={ep.y + 8} dur="2s" begin={`${i * 0.35}s`} repeatCount="indefinite" />
                <animate attributeName="opacity" values="0;0.7;0.7;0" keyTimes="0;0.1;0.85;1" dur="2s" begin={`${i * 0.35}s`} repeatCount="indefinite" />
              </circle>
            </g>
          ))}
        </svg>
    ...
    <div className="relative h-full overflow-hidden rounded-lg bg-[#0a0a0a]">
      ...
      >
        ...
        {layers.map((layer, i) => (
          <g key={layer.label}>
            {/* Layer bar */}
            <rect x="20" y={layer.y} width="160" height="18" rx="3"
              fill="rgba(255,255,255,0.03)"
              stroke={`rgba(222,145,166,${0.15 - i * 0.02})`}
              strokeWidth="0.5"
            />
            {/* Layer label */}
            <text x="28" y={layer.y + 12} className="font-mono" fontSize="7"
              fill={layer.color}>
              {layer.label}
            </text>
            {/* Open source badge */}
            <text x="172" y={layer.y + 12} textAnchor="end" className="font-mono" fontSize="5"
              fill="rgba(255,255,255,0.15)">
              open
            </text>
            {/* Data packet traveling through layer */}
            <circle r="2" cy={layer.y + 9} fill={layer.color}>
              <animate attributeName="cx" from="20" to="180" dur={layer.speed}
                begin={`${i * 0.8}s`} repeatCount="indefinite" />
              <animate attributeName="opacity" values="0;0.8;0.8;0"
                keyTimes="0;0.05;0.9;1" dur={layer.speed}
                begin={`${i * 0.8}s`} repeatCount="indefinite" />
            </circle>
            {/* Second staggered packet */}
            <circle r="1.5" cy={layer.y + 9} fill={layer.color}>
              <animate attributeName="cx" from="20" to="180" dur={layer.speed}
                begin={`${i * 0.8 + parseFloat(layer.speed) / 2}s`} repeatCount="indefinite" />
        ...
        {[60, 100, 140].map((x) => (
          <line key={x} x1={x} y1="10" x2={x} y2="103"
            stroke="rgba(222,145,166,0.06)" strokeWidth="0.5" strokeDasharray="2 3" />
        ))}
      </svg>
      ...
      <div className="absolute bottom-2.5 right-3 font-mono text-[9px]" style={{ color: "rgba(222,145,166,0.5)" }}>
        Open-source
      </div>
    </div>
  );
...
function EmbodySolutionVisual() {
  const facePts: [number, number][] = [
    [100, 35], [85, 42], [115, 42], // forehead, brow L, brow R
    [90, 52], [110, 52],             // eye L, eye R
    [100, 58],                       // nose
    [93, 68], [100, 72], [107, 68],  // mouth L, chin, mouth R
    [78, 48], [122, 48],             // temple L, temple R
    [82, 62], [118, 62],             // cheek L, cheek R
    [100, 42],                       // bridge
    [88, 48], [112, 48],             // inner brow
  ];
  ...
  return (
    <div className="relative h-full overflow-hidden rounded-lg bg-[#0a0a0a]">
      ...
      <div
        ...
        style={{ left: "35%", top: "20%", background: "linear-gradient(135deg, #a78bfa, #34d399)" }}
      />
      ...
      >
        ...
        <g>
          ...
          {faceLines.map(([a, b], i) => (
            <line key={i}
              x1={facePts[a][0]} y1={facePts[a][1]}
              x2={facePts[b][0]} y2={facePts[b][1]}
              stroke="rgba(167,139,250,0.25)" strokeWidth="0.5"
            >
              <animate attributeName="stroke-opacity"
                values="0.2;0.35;0.2" dur="3s"
          ...
          {facePts.map(([x, y], i) => (
            ...
            >
              <animate attributeName="r" values="1;1.6;1" dur="2.5s"
                begin={`${i * 0.15}s`} repeatCount="indefinite" />
              <animate attributeName="fill-opacity" values="0.4;0.8;0.4" dur="2.5s"
                begin={`${i * 0.15}s`} repeatCount="indefinite" />
            </circle>
          ))}
          ...
          <circle cx="90" cy="52" r="3" fill="rgba(167,139,250,0.12)">
            <animate attributeName="r" values="3;3;0.5;3;3" dur="4s"
              keyTimes="0;0.45;0.5;0.55;1" repeatCount="indefinite" />
          </circle>
          ...
          <circle cx="90" cy="52" r="1" fill="rgba(167,139,250,0.7)">
            <animate attributeName="cx" values="89;91;90;88;90" dur="5s" repeatCount="indefinite" />
            <animate attributeName="cy" values="52;51;52;53;52" dur="7s" repeatCount="indefinite" />
          </circle>
          <circle cx="110" cy="52" r="1" fill="rgba(167,139,250,0.7)">
            <animate attributeName="cx" values="109;111;110;108;110" dur="5s" repeatCount="indefinite" />
            <animate attributeName="cy" values="52;51;52;53;52" dur="7s" repeatCount="indefinite" />
          </circle>

          {/* Mouth animation — speaking */}
          <path d="M93 68 Q100 72 107 68" stroke="rgba(167,139,250,0.3)" strokeWidth="0.8">
        ...
        <line x1="60" y1="30" x2="140" y2="30"
          ...
          <animate attributeName="y1" values="25;95;25" dur="4s" repeatCount="indefinite" />
          <animate attributeName="y2" values="25;95;25" dur="4s" repeatCount="indefinite" />
          <animate attributeName="stroke-opacity" values="0;0.3;0.3;0" dur="4s"
            keyTimes="0;0.1;0.9;1" repeatCount="indefinite" />
        </line>
        ...
        <g stroke="rgba(167,139,250,0.3)" strokeWidth="0.6">
          ...
          <path d="M68,82 L68,77 M68,82 L73,82">
            <animate attributeName="stroke-opacity" values="0.2;0.5;0.2" dur="2s" begin="1s" repeatCount="indefinite" />
          </path>
          {/* Bottom-right */}
          <path d="M132,82 L132,77 M132,82 L127,82">
            <animate attributeName="stroke-opacity" values="0.2;0.5;0.2" dur="2s" begin="1.5s" repeatCount="indefinite" />
          </path>
        </g>

        {/* Outer presence ring — rotating */}
        <circle cx="100" cy="55" r="38" stroke="rgba(167,139,250,0.08)" strokeWidth="0.5"
          strokeDasharray="8 4">
          <animateTransform attributeName="transform" type="rotate"
            values="0 100 55;360 100 55" dur="12s" repeatCount="indefinite" />
        </circle>
        <circle cx="100" cy="55" r="45" stroke="rgba(167,139,250,0.04)" strokeWidth="0.5"
          strokeDasharray="6 6">
          <animateTransform attributeName="transform" type="rotate"
            values="360 100 55;0 100 55" dur="18s" repeatCount="indefinite" />
        </circle>

        {/* Data particles — float upward */}
        {[[75, 90], [125, 85], [68, 65], [132, 70], [80, 38], [120, 35]].map(([x, y], i) => (
          <circle key={`dp-${i}`} cx={x} cy={y} r="0.8" fill="rgba(167,139,250,0.5)">
      ...
      <div className="absolute bottom-2.5 left-3 flex items-center gap-2">
        ...
        <span className="font-mono text-[9px] text-purple-400/60">Agent Active</span>
      </div>
    ...
    <section className="relative py-24 lg:py-32">
      ...
      <Container>
        ...
        >
            ...
            {useCases.map((useCase) => (
              ...
              >
                <div className="block h-full overflow-hidden rounded-xl border border-white/[0.07] bg-[#1a1a1a]">
                  <div className={`p-2.5 pb-0 ${useCase.colSpan === 4 ? "h-[180px] lg:h-[240px]" : "h-[180px]"}`}>
                    <useCase.Visual />
                  </div>
                  <div className="px-5 py-5">
                    <h3 className="text-lg font-medium text-white/90">{useCase.title}</h3>
          ...
          <motion.div variants={fadeUp} transition={{ duration: 0.4 }} className="mt-16">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {[
              {
                name: "Daydream",
                capability: "Turn a live camera feed into AI-generated video, in real time.",
                Visual: DaydreamSolutionVisual,
              },
              {
                name: "Frameworks",
                capability: "Stream to any device, any format, any scale, from a single source.",
                Visual: FrameworksSolutionVisual,
              },
              {
                name: "Streamplace",
                capability: "Open-source video infrastructure powering AT Protocol social apps.",
                Visual: StreamplaceSolutionVisual,
              },
              {
                name: "Embody",
                capability: "Deploy AI avatars that see, speak, and respond in real time.",
