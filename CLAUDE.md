...
| Contribute | `/contribute` | **no mockup** — replaces the Foundation's Grants & Funding Mechanisms Notion page as the canonical destination. Answers "how can I get involved?" in two parts: a sentence and a Discord button (in-repo copy), and the **funding ladder** — every way work gets paid for, ordered by size, read from the Notion *Funding paths* database. The roadmap's "Not on the roadmap?" block points here rather than straight at the forum. The hero's ground is a WebGPU dither field drawn with **vgpu** (`components/livepeer-ui/contribute-field.tsx` and its `.wgsl`) — the blog card art's dither grammar, moving, grey with the brand green where it peaks: colour from the theme tokens, one still frame under reduced motion, DPR capped at 1.5, and nothing at all where no WebGPU adapter comes up — the hero has to read the same without it. `.wgsl` imports go through `@vgpu/wgsl/loader-webpack`, wired in `next.config.ts` for both bundlers, with the type shim in `wgsl-env.d.ts`; the loader does not validate, so check a shader with `pnpm exec vgpu check <file> --require-validation` |
