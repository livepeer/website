...
**Templated content that gets updated frequently** goes to Notion instead — many records sharing one shape, maintained by people on their own schedule. Two surfaces do: the **roadmap register**, edited by people across several organisations who do not open pull requests and where a commitment a week stale is worse than no page at all, and the **blog**, where publishing should not require a deploy. Scope is those two — this is not a general migration of the site into a CMS, and bespoke page copy stays here. See below.
