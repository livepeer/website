...
`date` (yyyy-mm-dd), `health` (`on track`, `at risk` or `off track`), `summary`
(the update in one line), optional `author` (`{ name, avatar }`, where `avatar`
is a file in `public/people`), and optional `draft`. The body is the write-up,
and may be empty. The filename is not read; date it so the folder sorts.
